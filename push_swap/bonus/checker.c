...
/*   Updated: 2023/06/06 12:55:39 by mstocche         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "checker.h"

void	check_duplicates(t_listlink *stack_a, int argc, int *error)
{
	int	*seen;
	int	i;
	int	j;
...
void	check_int(char **helper, t_listlink *stack_a, int flag, int *error)
{
	int	i;
	int	j;
		...
		while (helper[i][j])
		{
			if (helper[i][j] != '-' && helper[i][j] != '+')
				if (helper[i][j] < 48 || helper[i][j] > 57)
					ft_error1(helper, stack_a, flag, error);
			j++;
		}
		i++;
	}
...
void	create_stack_a2(t_listlink **stack_a, char **argv,
	t_create *utils, int argc, int *error)
{
	...
	while (utils->i < argc)
	{
		...
		check_int(utils->helper, *stack_a, 2, error);
		utils->j = 0;
...
t_listlink	*create_stack_a(char **argv, int argc, int *error)
{
	t_listlink	*stack_a;
	t_create	utils;
	...
	utils.index = 0;
	utils.helper = ft_split(argv[utils.i++], ' ');
	check_int(utils.helper, NULL, 1, error);
	...
	create_stack_a2(&stack_a, argv, &utils, argc, error);
	return (stack_a);
}
...
int	main(int argc, char **argv)
{
	...
	stack_a = create_stack_a(argv + 1, argc - 1, &error);
	argc = stack_a->before->index + 1;
	check_duplicates(stack_a, argc, &error);
	if (error)
	{		
		if (get_next_line(0) == NULL)
		{
